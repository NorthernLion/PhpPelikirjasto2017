...
class GameController extends BaseController {
    ...
    public static function store() {
        ...
        if (count($errors) == 0) {
            ...
            View::make('game/game_create.html', array('errors' => $errors, 'game' => $game));
        }
    }
    ...
    public static function edit($id) {
        if (!self::is_admin()) {
            Redirect::to('/game/' . $id, array('message' => 'Vain admin voi muokata peliä!'));                   
        } else {
            $game = Game::find($id);
...
//Pelin muokkauksen tallennus
    public static function update($id) {
        if (!self::is_admin()) {
            View::make('game/game_modify.html', array('message' => 'Vain admin voi muokata peliä!'));
        } else {
            self::check_logged_in();
            $params = $_POST;
...
//Pelin poisto
    public static function destroy($id) {
        if (self::is_admin()) {
            ...
            Redirect::to('/game/' . $id, array('message' => 'Vain admin voi tuhota pelin!'));
        }
    }
